...
foreach ($cmd in @("python3", "node", "npm")) {
    if (-not (Get-Command $cmd -ErrorAction SilentlyContinue)) {
        Write-Err "'$cmd' est introuvable. Installe-le avant de continuer."
    }
}
...
if (Test-Path $logFile) {
    ...
    Write-Ok "Logs analysés — ERRORs : $nbErr | CRITICALs : $nbCrit"

    if ($nbCrit -gt 0) {
        Write-Host "[ALERTE] $nbCrit ligne(s) CRITICAL trouvée(s) :" -ForegroundColor Red
        Select-String "CRITICAL" $logFile | ForEach-Object {
            Write-Host "  -> $($_.Line)" -ForegroundColor Red
        }
    }
} else {
    Write-Warn "Fichier de log '$logFile' introuvable — analyse ignorée"
}
