...
"RAPPORT D'ANALYSE - $(Get-Date -Format 'dd/MM/yyyy HH:mm')" |
    Out-File $rapportPath -Encoding UTF8
...
Select-String "CRITICAL" $LogFile |
    ...
    ForEach-Object {
        $derniersCritiques += "Ligne $($_.LineNumber) : $($_.Line)`n"
    }

Ecrire-Rapport "Derniers incidents critiques" $derniersCritiques $rapportPath
