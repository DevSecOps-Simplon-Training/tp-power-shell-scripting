...
# Fonction Test-Fichier qui vérifie l'existence d'un fichier
function Test-Fichier {
    param([string]$Fichier, [string]$Nom)
    if (Test-Path $Fichier) {
        Write-Ok "Fichier trouvé : $Nom"
    } else {
        Write-Warn "Fichier manquant : $Nom"
    }
}
...
Test-Fichier "config.json" "config.json"
Test-Fichier "ressources/server.log" "server.log"
