...
if (Test-Path "python-api/requirements.txt") {
    ...
    Write-Warn "Fichier python-api/requirements.txt introuvable : installation Python ignorée"
}
...
if (Test-Path "node-client/package.json") {
    ...
    Write-Warn "Fichier node-client/package.json introuvable : installation Node ignorée"
}
...
if (Test-Path $logFile) {
    ...
    Write-Ok "Logs analyses : ERROR=$nbErr ; CRITICAL=$nbCrit"

    if ($nbCrit -gt 0) {
        Write-Host "Incidents critiques détectés :" -ForegroundColor Red

        Select-String "CRITICAL" $logFile |
            ForEach-Object {
                Write-Host $_.Line -ForegroundColor Red
            }
    }
} else {
    Write-Warn "Fichier log introuvable : $logFile"
}

Write-Host ""
Write-Banner "SETUP TERMINE AVEC SUCCES" "Green"
Write-Host ""
