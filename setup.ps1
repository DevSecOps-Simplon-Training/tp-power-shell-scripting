...
foreach ($cmd in @("python3", "node", "npm", "git")) {
    if (-not (Get-Command $cmd -ErrorAction SilentlyContinue)) {
        Write-Err "Prérequis manquant : $cmd non trouvé"
    }
}
...
if (Test-Path $logFile) {
    ...
    if ($nbCrit -gt 0) {
        Write-Host "ALERTE CRITIQUE : $nbCrit incident(s) critique(s) détecté(s) !" -ForegroundColor Red
        Write-Host ""
        Write-Output "Détails des incidents critiques :"
        Select-String "CRITICAL" $logFile | ForEach-Object { Write-Output $_.Line }
    }
}
else {
